...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .five-wrap{position:absolute;left:50%;transform:translateX(-50%);width:min(460px,88%);top:24%;background:rgba(8,18,33,.82);border:2px solid rgba(244,208,63,.45);border-radius:22px;padding:14px 20px 18px;z-index:12;backdrop-filter:blur(2px)}
    .five-row{display:flex;align-items:center;justify-content:center;gap:8px;padding:9px 6px;border-radius:10px;margin:3px 0}
    .five-row:nth-child(odd){background:rgba(255,255,255,.05)}
    .five-seg{font-size:clamp(24px,3.2vw,34px);font-weight:900;color:#fff;white-space:nowrap}
    .five-input{width:76px;font-size:28px;font-weight:900;text-align:center;border-radius:12px;border:2.5px solid rgba(244,208,63,.7);background:rgba(255,255,255,.08);color:#fff;padding:7px 4px;outline:none;font-family:'Fredoka One',cursive;transition:border-color .15s,background .15s}
...
<body>
  ...
  function renderSymbolCalc(stage){
    ...
    // Always rotate by round — ignore AI-provided items (AI always picks the same 3)
    const fallbackA = defaultKeys[roundOffset % defaultKeys.length];
    const fallbackB = defaultKeys[(roundOffset + 1) % defaultKeys.length];
    const fallbackC = defaultKeys[(roundOffset + 2) % defaultKeys.length];
    const uniq = [];
  ...
  function renderFindUnknown(stage){
    ...
    const defaultKeys = ["shell_blue","banana","stone_blue","cherry","mushroom","pearl_blue","starfish","butterfly","coral","jellyfish","candy","lollipop","hibiscus","leaf","bamboo"];
    const roundOffset = ((state.stagePracticeDone || 0) * 3) % defaultKeys.length;
    // Always rotate by round — ignore AI-provided items
    const fallbackA = defaultKeys[roundOffset % defaultKeys.length];
    const fallbackB = defaultKeys[(roundOffset + 1) % defaultKeys.length];
    const fallbackC = defaultKeys[(roundOffset + 2) % defaultKeys.length];
    const uniq = [];
